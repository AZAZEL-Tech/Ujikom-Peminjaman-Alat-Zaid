                            ...
                            @forelse($peminjamans as $p)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $p->alat->nama_alat ?? 'Alat Dihapus' }} <br>
                                        <span class="text-xs text-gray-400">Jml: {{ $p->jumlah }} Unit</span>
                                    </td>
                                    
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $p->tanggal_pinjam }} <span class="text-xs text-gray-400 mx-1">s/d</span> {{ $p->tanggal_kembali }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        ...
                                        @elseif($p->status == 'kembali')
                                            ...
                                            <div class="text-xs text-gray-400 mt-1">Kembali: {{ $p->tgl_dikembalikan }}</div>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        @if($p->status == 'menunggu')
                                            <div class="flex justify-end gap-2">
                                                ...
                                                <form action="{{ route('petugas.reject', $p->id) }}" method="POST">
                                                    @csrf @method('PATCH')
                                                    <button onclick="return confirm('Yakin tolak permintaan ini?')" class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-xs font-bold transition">
                                                        ✗ TOLAK
                                                    </button>
                                                </form>
                                            </div>
                                        @elseif($p->status == 'disetujui')
                                            <form action="{{ route('petugas.return', $p->id) }}" method="POST">
                                                @csrf @method('PATCH')
                                                <button onclick="return confirm('Proses pengembalian barang ini?')" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-xs font-bold transition">
                                                    PROSES PENGEMBALIAN
                                                </button>
                                            </form>
                                        @else
