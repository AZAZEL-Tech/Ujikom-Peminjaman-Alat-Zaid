            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                    ...
                    <form action="{{ route('dashboard') }}" method="GET" class="flex gap-2 w-full md:w-auto">
                        ...
                        @if (request('search'))
                            <a href="{{ route('dashboard') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded text-sm flex items-center">
                                Reset
                            </a>
                        @endif
                    </form>
                    ...
                    @forelse ($alats as $alat)
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border border-gray-200">
                            <div class="flex justify-between items-start mb-4">
                                ...
                                <span class="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded font-bold">Stok: {{ $alat->stok }}</span>
                            </div>
                            ...
                            <form action="{{ route('pinjam.store') }}" method="POST">
                                ...
                                <div class="mb-3">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Jumlah Pinjam (Maks: {{ $alat->stok }})</label>
                                ...
                                @if ($alat->stok > 0)
                                    <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded text-sm transition">
                                        PINJAM
                                    </button>
                                @else
